...
export const BookingPage = () => {
  ...
  const durations = [
    ...
    { value: '4', label: t('4 hours') },
    { value: '8', label: t('Full day (8 hours)') }
  ];
  ...
  const validateForm = () => {
    ...
    if (!formData.eventType) newErrors.eventType = t('Please select an event type');
    if (!formData.eventDate) newErrors.eventDate = t('Please select a date');
    if (!formData.startTime) newErrors.startTime = t('Please select a start time');
    if (!formData.duration) newErrors.duration = t('Please select duration');
    if (!formData.guestCount) {
      newErrors.guestCount = t('Please enter guest count');
    } else if (parseInt(formData.guestCount) < 1) {
      newErrors.guestCount = t('Guest count must be at least 1');
    } else if (selectedEventType && parseInt(formData.guestCount) > selectedEventType.maxCapacity) {
      newErrors.guestCount = `${t('Maximum capacity for')} ${selectedEventType.name} ${t('is')} ${selectedEventType.maxCapacity}`;
    }
    ...
    if (formData.eventDate) {
      ...
      if (selectedDateTime < today) {
        newErrors.eventDate = t('Cannot book events in the past');
      }
    }
            ...
            {selectedEventType ? (
              <div className="space-y-4 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-400">{t('Event Type')}:</span>
                ...
                {formData.startTime && (
                  <div className="flex justify-between">
                    <span className="text-slate-400">{t('Start Time')}:</span>
                ...
                <div className="pt-4 border-t border-slate-700">
                  <div className="flex justify-between text-base">
                    <span className="text-slate-200 font-semibold">{t('Estimated Total')}:</span>
                    <span className="text-brand-400 font-bold text-lg">
                      ${calculatePrice().toLocaleString()}
                    </span>
                  </div>
                  <p className="mt-2 text-xs text-slate-400">
                    {t('Final price may vary based on additional services and requirements.')}
                  </p>
                </div>
              </div>
            ) : (
              <p className="text-sm text-slate-400 text-center py-8">
                {t('Select an event type to see pricing details')}
              </p>
            )}
          </Card>

          <Card title={t('Need Help?')} className="border-slate-700/50">
            <p className="text-sm text-slate-300 mb-3">
              {t("Have questions about your booking? We're here to help!")}
            </p>
            <div className="space-y-2 text-xs text-slate-400">
              <p>📧 {t('Email')}: bookings@venue.com</p>
              <p>📞 {t('Phone')}: +1 (555) 123-4567</p>
              <p>🕒 {t('Hours')}: {t('Mon-Fri, 9 AM - 6 PM')}</p>
            </div>
          </Card>
        </div>
      </div>
