  ...
  return (
    <div className="space-y-8">
        ...
        {eventTypes.map((event) => (
          <Card
            ...
            subtitle={t('Starting at ${amount}', {
              amount: event.basePrice.toLocaleString()
            })}
            className="border-slate-700/50 hover:border-brand-500/50 transition-colors"
          >
            <div className="flex h-full flex-col">
              <p className="text-sm text-slate-300 mb-4">{t(event.description)}</p>

              <div className="space-y-3">
                <div className="text-xs text-slate-400">
                  <span className="font-semibold text-slate-200">{t('Max Capacity:')}</span>{' '}
                  {event.maxCapacity} {t('guests')}
                </div>

                <div>
                  <p className="text-xs font-semibold text-slate-200 mb-2">{t('Includes:')}</p>
      ...
      <Card 
        ...
        subtitle={t('Enhance your event')}
        className="border-brand-500/40"
      >
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 text-sm text-slate-300">
          <div>
            <p className="font-semibold text-slate-100">{t('Catering')}</p>
            <p className="text-xs text-slate-400 mt-1">
              {t('Professional catering services with customizable menus')}
            </p>
          </div>
          <div>
            <p className="font-semibold text-slate-100">{t('Photography')}</p>
            <p className="text-xs text-slate-400 mt-1">
              {t('Professional photography and videography packages')}
            </p>
          </div>
          <div>
            <p className="font-semibold text-slate-100">{t('Entertainment')}</p>
            <p className="text-xs text-slate-400 mt-1">
              {t('Live music, DJ services, and entertainment options')}
            </p>
          </div>
          <div>
            <p className="font-semibold text-slate-100">{t('Decoration')}</p>
            <p className="text-xs text-slate-400 mt-1">
              {t('Full decoration services matching your theme')}
            </p>
          </div>
        </div>
      </Card>
      ...
      <div className="text-center py-6">
        <p className="text-sm text-slate-300 mb-4">
          {t('Ready to book your event?')}
        </p>
        <Link to="/booking">
          <Button size="lg" variant="primary">
            {t('Start Booking Process')}
          </Button>
        </Link>
      </div>
    </div>
